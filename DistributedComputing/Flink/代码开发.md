...
# Quick Start
## Installation
最简单的引入Flink依赖项的方式就是利用Maven或者Gradle:
```
...
```
不过需要注意的是，由于Scala 2.11编译版本与2.10版本无法兼容，因此在Flink的依赖项后面也加了个后缀来表示使用的Scala版本，你可以选择需要的Scala版本进行操作。
## WordCount
Flink有个方便的地方就是能够直接在本地运行而不需要提交到集群上，下面的测试程序直接右键点击Run即可。
### Stream
这里展示的是基本的将
```
